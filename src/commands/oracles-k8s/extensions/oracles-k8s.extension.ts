...
declare module "@devnet/state" {
  export interface StateInterface {
    getOraclesK8sBuildMultiImage<M extends boolean = true>(must?: M,): Promise<M extends true
      ? OraclesK8sStateBuildMultiImage
      : Partial<OraclesK8sStateBuildMultiImage>>;
    ...
    isOraclesK8sBuildMultiImageReady(): Promise<boolean>;
    isOraclesK8sImageReady(): Promise<boolean>;
    isOraclesK8sRunning(): Promise<boolean>;

    removeOraclesK8sState(): Promise<void>;

    updateOraclesK8sBuildMultiImage(state: OraclesK8sStateBuildMultiImage): Promise<void>;
...
export const OraclesK8sStateBuildMultiImage = z.object({
  image: z.string(),
  registryHostname: z.string(),
  accountingTag: z.string(),
  ejectorTag: z.string(),
  csmTag: z.string(),
});

export type OraclesK8sStateBuildMultiImage = z.infer<typeof OraclesK8sStateBuildMultiImage>;
...
export const OraclesK8sState = z.object({
  ...
  buildMultiImage: OraclesK8sStateBuildMultiImage.optional(),
...
export const oraclesK8sExtension = (dre: DevNetRuntimeEnvironmentInterface) => {
  ...
  dre.state.updateOraclesK8sBuildMultiImage = (async function (stateImage: OraclesK8sStateBuildMultiImage) {
    const state = await dre.state.getOraclesK8sState(false);
    await dre.state.updateProperties("oraclesK8s", { ...state, buildMultiImage: stateImage });
  });
  ...
  dre.state.isOraclesK8sBuildMultiImageReady = (async function () {
    const state = await dre.state.getOraclesK8sBuildMultiImage(false);
    return state && !isEmptyObject(state) && (state.image !== undefined);
  });
  ...
  dre.state.getOraclesK8sBuildMultiImage = (async function <M extends boolean = true>(must: M = true as M) {
    return dre.state.getProperties(
      {
        image: "oraclesK8s.buildMultiImage.image",
        registryHostname: "oraclesK8s.buildMultiImage.registryHostname",
        accountingTag: "oraclesK8s.buildMultiImage.accountingTag",
        ejectorTag: "oraclesK8s.buildMultiImage.ejectorTag",
        csmTag: "oraclesK8s.buildMultiImage.csmTag",
      },
      "oraclesK8s",
      OraclesK8sStateBuildMultiImage,
      must,
