...
export const validatorClientIngressTmpl =  async (
  ...
  index: number,
    ...
    kind: "Ingress",
    metadata: {
      name: `lido-devnet-validator-client-ingress-${index}`,
    ...
    spec: {
      ...
      rules: [
        {
          host: `${process.env.GLOBAL_INGRESS_HOST_PREFIX}-validator${index > 0 ? index : ''}.${dre.network.name}.valset-02.testnet.fi`,
          http: {
            paths: [
              {
                path: "/",
