...
export const ChainKurtosisUp = command.isomorphic({
  description:
    "Starts the chain using Kurtosis (default legacy mode)",
  params: { preset: Params.string({ description: "Kurtosis config name." }) },
  async handler({ dre, dre: { logger, state }, params: { preset } }) {
    ...
    await state.updateChainMode("kurtosis");
    logger.log("Chain started in kurtosis mode");
  },
});
