...
export const LateProverBotK8sDown = command.cli({
  ...
  async handler({ dre, dre: { services: { lateProverBot }, logger, state }, params  }) {
    if (!(await state.isLateProverBotK8sRunning()) && !(params.force)) {
    ...
    await state.removeLateProverBotK8sState();
  },
});
